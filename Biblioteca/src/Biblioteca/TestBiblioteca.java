	...
	public static void main(String[] args) {
		...
		for(Utente u2:b.getListaUtenti()){
			...
			for(Prestito p:u2.getLibriInPrestito()){
				System.out.println(p.getL().getTitolo()+" "+p.getL().getAutore()+" copieDisp: "+p.getL().getCopieDisp()+" copieTot: "+p.getL().getCopieTot()+" NS: "+p.getL().getSerialNumber()+p.getDataPrestito());
			}
...
for(Utente u3:b.getListaUtenti()){
			...
			for(Prestito p1:u3.getLibriInPrestito()){
				System.out.println(p1.getL().getTitolo()+" "+p1.getL().getAutore()+" copieDisp: "+p1.getL().getCopieDisp()+" copieTot: "+p1.getL().getCopieTot()+" NS: "+p1.getL().getSerialNumber()+p1.getDataPrestito());
			}
